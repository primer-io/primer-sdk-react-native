...
import { PaymentOutcomeTransitioner, QrCodeTransitioner } from '../../Components/internal/checkout-flow/transitioners';
...
describe('QrCodeTransitioner', () => {
  it('navigates to the QR screen when a QR payload arrives', () => {
    const { value, replace } = makeNav();
    act(() => {
      create(wrap({ ...baseContext, qrCode: { base64: 'QR' } }, value, createElement(QrCodeTransitioner)));
    });
    expect(replace).toHaveBeenCalledWith(CheckoutRoute.qrCode);
  });

  it('re-navigates for a second QR payment after qrCode resets to null', () => {
    const { value, replace } = makeNav();
    let root: { update: (el: ReactNode) => void } | undefined;
    act(() => {
      root = create(wrap({ ...baseContext, qrCode: { base64: 'QR1' } }, value, createElement(QrCodeTransitioner)));
    });
    act(() => {
      root!.update(wrap({ ...baseContext, qrCode: null }, value, createElement(QrCodeTransitioner)));
    });
    act(() => {
      root!.update(wrap({ ...baseContext, qrCode: { base64: 'QR2' } }, value, createElement(QrCodeTransitioner)));
    });
    expect(replace).toHaveBeenCalledTimes(2);
    expect(replace).toHaveBeenNthCalledWith(1, CheckoutRoute.qrCode);
    expect(replace).toHaveBeenNthCalledWith(2, CheckoutRoute.qrCode);
  });
});
