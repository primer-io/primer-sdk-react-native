...
class RNTPrimerHeadlessUniversalPaymentMethodNativeUIManager: RCTEventEmitter {
  ...
  func configure(
    ...
    do {
      self.paymentMethodNativeUIManager = try PrimerSDK.PrimerHeadlessUniversalCheckout
        .NativeUIManager(paymentMethodType: paymentMethod)
      resolver(nil)
    } catch {
  ...
  func showPaymentMethod(
    _ intentStr: String, resolver: @escaping RCTPromiseResolveBlock,
    rejecter: @escaping RCTPromiseRejectBlock
  ) {
    do {
