    ...
    return (
      <View>
        {paymentMethodsResources.map(paymentMethodResource => {
          const testId = `button-${paymentMethodResource.paymentMethodType.toLowerCase().replace('_', '-')}`;
          const isNativeView = typeof paymentMethodResource.nativeViewName === 'string';
          return isNativeView ? (
            <TouchableOpacity testID={testId} key={paymentMethodResource.paymentMethodType}>
              <NativeResourceView
                onPress={() => {
                  paymentMethodButtonTapped(paymentMethodResource.paymentMethodType);
                }}
